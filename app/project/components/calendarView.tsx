...
export default function CalendarView({ project, events, setEvents }: any) {
  ...
  const handleSaveSchedule = async (taskData: any) => {
    try {
      ...
      if (res.success) {
        setEvents((prev: any) => [...prev, { ...res.data, id: String(res.data.id) }]);
  ...
  const handleUpdateTask = async (taskData: any) => {
    ...
    try {
      ...
      const res = await updateProjectTask(project.id, Number(selectedTask.id), taskData);
      
      if (res.success) {
        setEvents((prev: any) => 
          prev.map((e: any) => (Number(e.id) === Number(selectedTask.id) ? { ...res.data, id: String(res.data.id) } : e))
        );
        setIsModalOpen(false);
    ...
    <div className={styles.container}>
      <div className={styles.topControls}>
        <div className={styles.viewToggle}>
          <button className={viewMode === 'month' ? styles.activeTab : ''} onClick={() => setViewMode('month')}>월별 보기</button>
          <button className={viewMode === 'sprint' ? styles.activeTab : ''} onClick={() => setViewMode('sprint')}>주간 스프린트</button>
        </div>
        <h2 className={styles.currentMonthTitle}>{viewMode === 'month' ? '월 일정' : '주간 스프린트'}</h2>
      </div>

      <div className={viewMode === 'month' ? styles.calendarWrapper : styles.timelineBoard}>
        {viewMode === 'month' ? (
          <FullCalendar
            ...
            <div key={member.id} className={styles.memberRow}>
              ...
              <div className={styles.memberCalendar}>
                <FullCalendar
                  ...
                  locale={koLocale}
                  height="auto"
                  events={calendarEvents.filter((e: any) => Number(e.assignedToId) === Number(member.id))}
                  dayHeaders={index === 0}
                  selectable={true}
                  select={handleDateSelect}
      ...
      {isModalOpen && (
        <Modal 
          ...
          onSave={modalMode === 'create' ? handleSaveSchedule : handleUpdateTask} // 💡 모드에 따른 핸들러 분기
        />
      )}
    </div>
  );
